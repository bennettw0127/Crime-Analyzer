...
namespace Crime_Analyzer
{
    public static class CrimeStatsLoader
    {
        ...
        public static Lists<CrimeStats> Load(string csvDataFilePath)
        {
            Lists<CrimeStats> crimeStatsList = new List<CrimeStats>();
            ...
            try
            {
                using (var reader = new StreamReader(csvDataFilePath))
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        {
                            throw new Exception($"Row {lineNymber} contains {values.Length} values. It should contain {NumItemsInRow}.");
                        }
                        try
                        {
                            ...
                            int aggravatedAssualt = Int32.Parse(values[6]);
                            ...
                            CrimeStats crimeStats = new CrimeStats(year, population, violentCrime, murder, rape, robbery, aggravatedAssualt, propertyCrime, burglary, theft, motorVehicleTheft);
